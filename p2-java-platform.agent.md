...
- Define an **OpenRewrite recipe strategy** for the repo (pilot on one module first, then scale), focused on safe mechanical upgrades.
...
- Keep **JAR** packaging, embedded Tomcat, and plugin changes **consistent** across modules when a shared parent exists — **WAR** is transitional only until each module migrates (`docs/DEEPWIKI.md`).
- Treat OpenRewrite as an accelerator, not a substitute for tests/integration validation.
